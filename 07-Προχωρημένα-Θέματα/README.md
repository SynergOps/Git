...
### Git Stash
Το stash σας επιτρέπει να αποθηκεύσετε προσωρινά τις αλλαγές σας χωρίς να κάνετε commit. Λειτουργεί ως μία στοίβα, επιτρέποντας να αποθηκευτούν πολλαπλές αλλάγες, επιτρέποντας ουσιαστικά να μετακινηθούμε σε διαφορετική φάση (π.χ. αλλαγή branch για διόρθωση ενός bug ή pull αλλαγών από απομακρυσμένο repository) χωρίς να χάσουμε τις αλλαγές μας.  
...
- **Checkout**: Μετακινεί το HEAD ή επαναφέρει αρχεία, χωρίς αλλαγή του history.

### Git Tags
Σκοπός ενός tag είναι η επισήμανση συγκεκριμένου commit με μία ετικέτα, για παράδειγμα σηματοδοτώντας μια έκδοση του project ή ένα milestone (π.χ. την εισαγωγή ενός νέου feature). Επιπλέον, τα tags δείχνουν πάντα στο ίδιο commit. 

## Git Reflogs

Το reflog καταγράφει όλες τις κινήσεις του HEAD, ακόμη κι εκείνες που δεν εμφανίζονται στο επίσημο history μέσω του git log (όπως checkouts, merges, rebases, stashes κλπ). Κατ' ουσία, δρα ως μία δικλείδα ασφαλείας του git, με την ευκαιρία ανάκλησης χαμένων commits μέχρι και την αναίρεση δράσεων όπως `git reset --hard` ή `git rebase`. Παρά ταύτα, το reflog δρα μόνο τοπικά: η ύπαρξή του δεν διατηρείται μέσω push ή clone σε κάποιο απομακρυσμένο repository, ούτε μπορεί να επαναφέρει χαμένες συνεισφορές άλλου ατόμου στο repository. 

## Git Bisect

Ας θεωρήσουμε την περίπτωση που προσπαθούμε να εντοπίσουμε ένα bug στο project μας, χωρίς να γνωρίζουμε σε ποιό commit εμφανίστηκε. Η εντολή bisect μας δίνει την δυνατότητα να εντοπίσουμε το προβληματικό αυτό commit μέσω αναζήτησης με διχοτόμηση (bisection). Δηλώνουμε ένα μη-προβληματικό (good) commit στο ιστορικό μας και το προβληματικό (bad) και διενεργούμε tests ή προσπαθούμε να αναπαράγουμε το bug, μέχρι να εντοπιστεί το προβληματικό commit.
